...
int main(int argc, char** argv) {
    if (argc < 4) {
        fprintf(stdout, "Usage: test_DLT_semi m bw loops\n");
        exit(0);
    }
    ...
    GenerateCosPmlTable(bw, m, cos_pml_table, workspace);
    // take transpose of the precomputed cosine series, necessary for the inverse transform
    TransposeCosPmlTable(bw, m, cos_pml_table, transpose_cos_pml_table);
    ...
    for (int k = 0; k < loops; ++k) {
        ...
        InvDLTSemi(coeffs, bw, m, samples, transpose_cos_pml_table, sin_values, workspace, &inverseDCT);
        total_time_i += (csecond() - time_start);

        time_start = csecond();
        // forward semi-naive transform
        DLTSemi(samples, bw, m, new_coeffs, workspace, cos_pml_table, weights, &forwardDCT);
