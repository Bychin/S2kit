/*
    ...
    Once the coefficients are generated, the corresponding function is synthesized using InvFST_semi_memo(),
    then transformed (analyzed) using FST_semi_memo(). Timing data is printed.
...
int main(int argc, char** argv) {
    ...
    double *seminaive_naive_tablespace, *trans_seminaive_naive_tablespace, *workspace;
    ...
    ****/
    ...
    if ((rdata == NULL) || (idata == NULL) || (rresult == NULL) || (iresult == NULL) || (rcoeffs == NULL) ||
        (icoeffs == NULL) || (seminaive_naive_tablespace == NULL) || (trans_seminaive_naive_tablespace == NULL) ||
        (workspace == NULL)) {
    ...
    trans_seminaive_naive_table = Transpose_SemiNaive_Naive_Pml_Table(seminaive_naive_table, bw, cutoff,
    ...
    */
    ...
    for (i = 0; i < loops; i++) {
        ...
        FST_semi_memo(rdata, idata, rresult, iresult, bw, seminaive_naive_table, workspace, 0, cutoff, &dctPlan,
                      &fftPlan, weights);
    ...
    fprintf(stdout, "Average r-o error:\t\t %.4e\t", granderror / ((double)loops));
    fprintf(stdout, "std dev: %.4e\n", stddev_error);
    fprintf(stdout, "Average (r-o)/o error:\t\t %.4e\t", grandrelerror / ((double)loops));
    fprintf(stdout, "std dev: %.4e\n\n", stddev_relerror);
        ...
        for (m = 0; m < bw; m++) {
            ...
            for (l = m; l < bw; l++) {
                ...
                fprintf(errorsfp, "dummy = %d\t m = %d\tl = %d\t%.10f  %.10f\n", dummy, m, l,
                        fabs(rcoeffs[dummy] - rresult[dummy]), fabs(icoeffs[dummy] - iresult[dummy]));
