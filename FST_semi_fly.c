...
void FST_semi_fly(double* rdata, double* idata, double* rcoeffs, double* icoeffs, int bw, double* workspace,
    ...
    for (m = 0; m < bw; m++) {
        ...
        if (m < cutoff) {
            ...
            /* do the real part */
            SemiNaiveReduced(rres + (m * size), bw, m, fltres, scratchpad, pmls, weights, dctPlan);
            ...
            /* do imaginary part */
            SemiNaiveReduced(ires + (m * size), bw, m, fltres, scratchpad, pmls, weights, dctPlan);
          ...
          fprintf(stderr,"m = %d\n",-(size-m));
            */
            ...
            if ((size - m) < cutoff) {
                ...
                /* do real part */
                SemiNaiveReduced(rres + (m * size), bw, size - m, fltres, scratchpad, pmls, weights, dctPlan);
                ...
                /* do imaginary part */
                SemiNaiveReduced(ires + (m * size), bw, size - m, fltres, scratchpad, pmls, weights, dctPlan);
    ...
    for (m = 0; m < bw; m++) {
        ...
        */
        ...
        if (m < cutoff) {
            ...
            /* do real part first */
            InvSemiNaiveReduced(rdataptr, bw, m, rinvfltres, pmls + TableSize(m, bw), sin_values, scratchpad, idctPlan);
...
void Conv2Sphere_semi_fly(double* rdata, double* idata, double* rfilter, double* ifilter, double* rres, double* ires,
    ...
    */
    ...
    FST_semi_fly(rdata, idata, frres, fires, bw, scratchpad, 1, bw, &dctPlan, &fftPlan, weights);

    FZT_semi_fly(rfilter, ifilter, filtrres, filtires, bw, scratchpad, 1, &dctPlan, weights);

    TransMult(frres, fires, filtrres, filtires, trres, tires, bw);

    InvFST_semi_fly(trres, tires, rres, ires, bw, scratchpad, 1, bw, &idctPlan, &ifftPlan);
