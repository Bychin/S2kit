...
void FST_semi_fly(double *rdata, double *idata, double *rcoeffs,
                  double *icoeffs, int bw, double *workspace, int dataformat,
                  int cutoff, fftw_plan *dctPlan, fftw_plan *fftPlan,
                  double *weights) {
  int size, m, i, j;
  int dummy0, dummy1;
  double *rres, *ires;
  ...
  for (m = 0; m < bw; m++) {
    ...
    if (m < cutoff) {
      ...
      /* do the real part */
      SemiNaiveReduced(rres + (m * size), bw, m, fltres, scratchpad, pmls,
                       weights, dctPlan);
      ...
      /* do imaginary part */
      SemiNaiveReduced(ires + (m * size), bw, m, fltres, scratchpad, pmls,
                       weights, dctPlan);
      ...
      /* do real part */
      Naive_AnalysisX(rres + (m * size), bw, m, weights, fltres, pmls,
                      scratchpad);
      memcpy(rdataptr, fltres, sizeof(double) * (bw - m));
      rdataptr += bw - m;

      /* do imaginary part */
      Naive_AnalysisX(ires + (m * size), bw, m, weights, fltres, pmls,
                      scratchpad);
      memcpy(idataptr, fltres, sizeof(double) * (bw - m));
      idataptr += bw - m;
    }
    ...
    fprintf(stderr,"m = %d\n",-(size-m));
      */
      ...
      if ((size - m) < cutoff) {
        ...
        Naive_AnalysisX(rres + (m * size), bw, size - m, weights, fltres, pmls,
                        scratchpad);
        ...
        /* do imaginary part */
        Naive_AnalysisX(ires + (m * size), bw, size - m, weights, fltres, pmls,
                        scratchpad);
...
void InvFST_semi_fly(double *rcoeffs, double *icoeffs, double *rdata,
                     double *idata, int bw, double *workspace, int dataformat,
                     int cutoff, fftw_plan *idctPlan, fftw_plan *ifftPlan) {
  int size, m, i, n;
  double *rdataptr, *idataptr;
  ...
  for (m = 0; m < bw; m++) {
    ...
    */
    ...
    if (m < cutoff) {
      ...
      /* do real part first */
      InvSemiNaiveReduced(rdataptr, bw, m, rinvfltres, pmls + TableSize(m, bw),
                          sin_values, scratchpad, idctPlan);

      /* now do imaginary part */
      InvSemiNaiveReduced(idataptr, bw, m, iminvfltres, pmls + TableSize(m, bw),
                          sin_values, scratchpad, idctPlan);
    ...
    for (m = bw + 1; m < size; m++) {
      ...
      */
      ...
      if ((size - m) < cutoff) {
        ...
        CosPmlTableGen(bw, size - m, pmls, scratchpad);
        /** now take transpose **/
        Transpose_CosPmlTableGen(bw, size - m, pmls,
                                 pmls + TableSize(size - m, bw));

        /* do real part first */
        InvSemiNaiveReduced(rdataptr, bw, size - m, rinvfltres,
                            pmls + TableSize(size - m, bw), sin_values,
                            scratchpad, idctPlan);
...
void FZT_semi_fly(double *rdata, double *idata, double *rres, double *ires,
                  int bw, double *workspace, int dataformat, fftw_plan *dctPlan,
                  double *weights) {
  ...
  /* do the real part */
  SemiNaiveReduced(r0, bw, 0, rres, scratchpad, pmls, weights, dctPlan);

  if (dataformat == 0) /* do imaginary part */
    SemiNaiveReduced(i0, bw, 0, ires, scratchpad, pmls, weights, dctPlan);
...
void Conv2Sphere_semi_fly(double *rdata, double *idata, double *rfilter,
  ...
  */
  ...
  FST_semi_fly(rdata, idata, frres, fires, bw, scratchpad, 1, bw, &dctPlan,
               &fftPlan, weights);

  FZT_semi_fly(rfilter, ifilter, filtrres, filtires, bw, scratchpad, 1,
               &dctPlan, weights);

  TransMult(frres, fires, filtrres, filtires, trres, tires, bw);

  InvFST_semi_fly(trres, tires, rres, ires, bw, scratchpad, 1, bw, &idctPlan,
                  &ifftPlan);
