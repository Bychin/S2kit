...
int main(int argc, char **argv) {
  ...
  transpose_cos_pml_table = (double *)malloc(sizeof(double) * TableSize(m, bw));
  ...
  */
  ...
  /* forward DCT */
  forwardDCT =
      fftw_plan_r2r_1d(2 * bw, workspace, weights, FFTW_REDFT10, FFTW_ESTIMATE);

  /* inverse DCT */
  inverseDCT =
      fftw_plan_r2r_1d(2 * bw, workspace, weights, FFTW_REDFT01, FFTW_ESTIMATE);
  ...
  */
  ...
  for (k = 0; k < loops; k++) {
    ...
    InvSemiNaiveReduced(coeffs, bw, m, samples, transpose_cos_pml_table,
                        sin_values, workspace, &inverseDCT);
    tstopi = csecond();
    totaltimei += (tstopi - tstarti);

    tstartf = csecond();
    /* now do forward semi-naive transform */
    SemiNaiveReduced(samples, bw, m, newcoeffs, workspace, cos_pml_table,
                     weights, &forwardDCT);
    tstopf = csecond();
  ...
  coefficients and the new ones
    */
    ...
    for (j = 0; j < bw - m; j++) {
      ...
      tmp_relerror = tmp_error / (fabs(coeffs[j]) + pow(10.0, -50.0));
  ...
  fprintf(stdout, "Average r-o error:\t\t %.4e\t", sum_error / ((double)loops));
