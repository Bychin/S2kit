...
const int BW_LIMIT = 812; // limit for bw is used in functions which calculate table size
...
/*
    ...
    Also, if bw > BW_LIMIT, the closed form
    ...
    for (int row = 0; row < bw; ++row) {
        ...
        if (row <= m) {
            if (!(row % 2))
                ...
                tableptr = cos_pml_table + (row / 2) + (m / 2) + 1;
        } else {
...
double** Spharmonic_Pml_Table(const int bw, double* resultspace, double* workspace) {
    ...
    CosPmlTableGen(bw, 0, spharmonic_pml_table[0], workspace);
...
double** Transpose_Spharmonic_Pml_Table(double** spharmonic_pml_table, const int bw, double* resultspace) {
    ...
    Transpose_CosPmlTableGen(bw, bw - 1, spharmonic_pml_table[bw - 1], transpose_spharmonic_pml_table[bw - 1]);
...
double** SemiNaive_Naive_Pml_Table(const int bw, const int m, double* resultspace, double* workspace) {
    ...
    CosPmlTableGen(bw, 0, seminaive_naive_table[0], workspace);
    ...
    if (m) {
        ...
        PmlTableGen(bw, m, seminaive_naive_table[m], workspace);
    }
...
double** Transpose_SemiNaive_Naive_Pml_Table(double** seminaive_naive_pml_table, const int bw, const int m,
    ...
    // need to load up the `trans_seminaive_naive_pml_table` by transposing
